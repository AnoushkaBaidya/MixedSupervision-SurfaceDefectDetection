...
class End2End:
    ...
    def training_iteration(self, data, device, model, criterion_seg, criterion_dec, optimizer, weight_loss_seg, weight_loss_dec,
        ...
        for sub_iter in range(num_subiters):
            ...
            # Assuming images[sub_iter] is a valid tensor
            #images_ = torch.stack(images[sub_iter * memory_fit:(sub_iter + 1) * memory_fit]).to(device)
            #seg_masks_ = torch.stack(seg_masks[sub_iter * memory_fit:(sub_iter + 1) * memory_fit]).to(device)
            #seg_loss_masks_ = torch.stack(seg_loss_masks[sub_iter * memory_fit:(sub_iter + 1) * memory_fit]).to(device)
            #is_pos_ = seg_masks_.max().reshape((memory_fit, 1)).to(device)
    ...
    def _train_model(self, device, model, train_loader, criterion_seg, criterion_dec, optimizer, validation_set, tensorboard_writer):
        ...
        for epoch in range(num_epochs):
            self._log(f"Starting Epoch {epoch + 1}/{num_epochs}", LVL_INFO)  # Print the current epocH
            if epoch % 5 == 0:
            ...
            with tqdm(enumerate(train_loader), total=len(train_loader), desc=f"Epoch {epoch + 1}/{num_epochs}") as t:
                for iter_index, data in t:
                    start_1 = timer()
                    #print(f"Data type at iteration {iter_index}: {type(data)}")
