...
# Configuration
MODEL_PATH = "/Users/anoushka/Desktop/Projects/MixedSupervision-SurfaceDefectDetection/RESULTS/TRAININGDATASET/experiment_1/models/final_state_dict.pth"  # Path to the saved model
...
# Export model to ONNX for inference optimization
onnx_path = "model.onnx"
dummy_input = torch.randn(1, INPUT_CHANNELS, INPUT_WIDTH, INPUT_HEIGHT).to(DEVICE)
...
logging.info("Starting image processing in batches...")
image_paths = [os.path.join(TEST_IMAGES_DIR, img_name) for img_name in os.listdir(TEST_IMAGES_DIR)]
for i in tqdm(range(0, len(image_paths), BATCH_SIZE)):
    ...
    with torch.no_grad():
        dec_out, seg_out = model(batch_images)
        img_scores = torch.sigmoid(dec_out).cpu().numpy().flatten()
        seg_preds = torch.sigmoid(seg_out).cpu().numpy()

    for j, img_name in enumerate(valid_image_names):
        label = 1 if "bad" in img_name.lower() else 0
        predictions.append(img_scores[j])
        ...
        # cv2.imwrite(seg_save_path, (seg_preds[j] * 255).astype(np.uint8))
